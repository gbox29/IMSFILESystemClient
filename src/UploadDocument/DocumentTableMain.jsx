...
export default function DocumentTableMain() {
    ...
    const handleSearch = () => {
        ...
        axios.get("//13.215.156.71/getDocuments/search", {
            params: {
                sName: sName,
                date: date
            }
    ...
    const handleDelete = (row_id) => {
        ...
        if (result) {
            axios.delete("//13.215.156.71/getDocuments", {
                params: {
                    id: row_id
                }
            }).then((response) => {
    ...
    useEffect(() => {
        const getResult = () => {
            axios.get("//13.215.156.71/getDocuments").then((response) => {
                setData(response.data.result[0]);
            }).catch(error => {
                console.error('AxiosError:', error);
            });
    ...
    const handleSubmit = () => {
        ...
        if (document) {
            axios.post('//13.215.156.71/upload', formData, {
                headers: {
